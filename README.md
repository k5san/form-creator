...
5. スプレッドシートを保存し、一度閉じ、再度開く。
6. メニューに「JSONからGoogleForm作成」が増えていることを確認する。
7. スクリプトの実行に許可が必要なので、内容を確認し、許可する。
...
- 3. JSONファイルがマイドライブ直下へアップロードされ、それを利用してフォーム作成まで行います。
	
### 3. アップロード済みのJSONを利用して作成する	
- 1. Google Driveのマイドライブ直下にJSONファイルを配置してください。
- 2. このスプレッドシートの上部メニューから「JSONからGoogleForm作成」→「アップロード済みJSONからフォーム作成」を選択します。
- 3. マイドライブ直下からJSONファイルを探査し、コンボリストに表示されるので、フォーム作成したいJSONを選択します。
- 4. 選択したJSONファイルを利用してフォーム作成を行います。

これらの操作はスプレッドシートを他のユーザーに共有した場合でも同じです。  
共有されたユーザーがアップロードやフォーム作成を行った場合、そのユーザー自身のマイドライブ直下が作業ディレクトリになります。
